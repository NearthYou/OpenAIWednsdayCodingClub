...
- 핵심 포인트
  ...
  - 곧 마감 카드의 잔여 시간 시각화
  - 굿즈 탐색 / 홈 검색 / 일정 상세가 한 흐름으로 연결됨

## 수요일 발표 이후 추가 구현

- 홈 / 상세 캘린더 / 굿즈 탐색 히어로 영역을 감성 카피 + 일러스트 카드 구조로 개선
- 일정 상세의 `내 캘린더 추가`를 `.ics` 다운로드 대신 로컬 `내 일정` 저장 흐름으로 변경
- 저장한 일정이 상세 캘린더 날짜 하이라이트와 마이 페이지 예정 일정 목록에 함께 반영되도록 연결
- 저장된 일정 상태에서는 CTA가 `상세 캘린더에서 보기`로 바뀌고, 누르면 해당 날짜가 보이도록 이동
- `곧 마감` 카드에 남은 시간에 따른 색상 단계와 `오늘 마감`, `D-n` 배지 추가
- 마이페이지를 저장 일정 중심 레이아웃으로 정리하고 README를 현재 구현 상태 기준으로 갱신

## 4분 데모 플로우

### 1. 문제와 서비스 소개

- 팬 활동 정보가 일정, 굿즈, 마감 기준으로 흩어져 있다는 문제를 짧게 소개
- 이 프로젝트가 그 정보를 한 흐름으로 연결하는 팬덤 일정 관리 앱이라고 설명

### 2. 홈 대시보드

- 홈에서 오늘의 덕질 문장, 구독 일정, 곧 마감 카드 확인
- `곧 마감` 카드가 마감일에 가까워질수록 색이 진해지고 배지가 붙는 점 강조

### 3. 일정 상세 -> 저장 -> 캘린더 반영

- 홈이나 캘린더에서 일정 상세로 이동
- `내 캘린더 추가` 클릭
- 다시 버튼을 눌러 `상세 캘린더에서 보기`로 이동
- 해당 날짜가 실제로 표시되는 흐름 시연

### 4. 마이페이지 / 굿즈 탐색

- 마이페이지에서 저장된 예정 일정이 모이는 구조 설명
- 굿즈 탐색 페이지에서 별도 필터 기반 탐색이 가능한 점 짧게 시연

### 5. 테스트와 검증

- 타입 체크와 빌드 검증
- 핵심 사용자 플로우 수동 테스트 케이스 수행 결과
...
1. 상세 일정 저장 후 캘린더 반영
   - 절차: 일정 상세에서 `내 캘린더 추가` 클릭
   - 기대 결과: 저장된 일정이 상세 캘린더 날짜 표시와 마이페이지 예정 일정에 반영됨
   - 확인 결과: 통과

2. 저장된 일정에서 상세 캘린더로 이동
   - 절차: 저장된 상태의 CTA `상세 캘린더에서 보기` 클릭
   - 기대 결과: 상세 캘린더로 이동하면서 해당 날짜가 선택됨
   - 확인 결과: 통과

3. 곧 마감 카드 상태 강조
   - 절차: 홈 대시보드의 `곧 마감` 카드 확인
   - 기대 결과: 마감일이 가까운 항목일수록 카드가 더 붉은 톤을 띠고, `오늘 마감` 또는 `D-n` 배지가 보임
   - 확인 결과: 통과

4. 굿즈 / 캘린더 히어로 레이아웃
   - 절차: 홈, 상세 캘린더, 굿즈 탐색 페이지 진입
   - 기대 결과: 카피와 일러스트 카드가 함께 보이고, 카드 요약 영역이 의도한 2열 배치로 표시됨
   - 확인 결과: 통과

5. 마이페이지 예정 일정 목록
   - 절차: 저장 일정이 있는 상태에서 마이페이지 진입 후 페이지네이션과 삭제 버튼 확인
   - 기대 결과: 저장 일정이 목록에 표시되고, 페이지 이동과 삭제가 동작함
   - 확인 결과: 통과

### 이번 검증에서 중점적으로 본 항목

- AI로 빠르게 만든 UI 변경이 실제 사용자 흐름을 깨지 않는지
- 저장 / 이동 / 목록 반영처럼 여러 페이지를 가로지르는 상태 연결이 안정적으로 동작하는지
- 시각 강조 요소가 과하지 않으면서도 정보 우선순위를 잘 전달하는지
